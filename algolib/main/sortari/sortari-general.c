...
fprintf(stdout, "Inainte de INSERTION SORT\n");
print_idata(iarray, size);

if (issort(iarray, size, sizeof(int), compare_int) != 0)
   return 1;

fprintf(stdout, "Dupa INSERTION SORT\n");
print_idata(iarray, size);

fprintf(stdout, "Inainte de QUICK SORT\n");
print_idata(qarray, size);

if (qksort(qarray, size, sizeof(int), 0, size - 1, compare_int) != 0)
   return 1;

fprintf(stdout, "Dupa QUICK SORT\n");
print_idata(qarray, size);

fprintf(stdout, "Inainte de MERGE SORT\n");
print_sdata(sarray, size);

if (mgsort(sarray, size, STRSIZ, 0, size - 1, compare_str) != 0)
   return 1;

fprintf(stdout, "Dupa MERGE SORT\n");
print_sdata(sarray, size);


fprintf(stdout, "Inainte de COUNTING SORT\n");
print_idata(carray, size);

if (ctsort(carray, size, 10) != 0)
   return 1;

fprintf(stdout, "Dupa COUNTING SORT\n");
print_idata(carray, size);

fprintf(stdout, "Inainte de RADIX SORT\n");
print_idata(rarray, size);

if (rxsort(rarray, size, 8, 10) != 0)
   return 1;

fprintf(stdout, "Dupa RADIX SORT\n");
print_idata(rarray, size);
