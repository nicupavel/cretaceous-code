...
int update_screen(mode *m, int display)
{
   ...
   if(m)
   {
	 ...
	 */
	 ...
	 case 'v':
	 case 'V': status = vu_screen(m->timer, display); break;
	 case 'n':
	 case 'N': status = sensors_screen(m->timer, display); break;
	 case 's':
...
int credit_screen(int rep, int display)
{
   ...
   if(first)
   {
      ...
      sock_send_string(sock,
		       "screen_set A name {Credits for LCDstats}\n");
	 ...
	 sock_send_string(sock,
			  "widget_set A two 1 3 { by Nicu Pavel}\n");
	 sock_send_string(sock,
			  "widget_set A three 1 4 { and iTuner Networks Corp}\n");
      }
      else
      {
	 sock_send_string(sock, "widget_add A text scroller\n");
	 sock_send_string(sock,
			  "widget_set A text 1 2 20 2 v 8 {     for Linux       by Nicu Pavel iTuner Networks Corp}\n");
      }
   }
