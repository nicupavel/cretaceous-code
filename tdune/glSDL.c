/*(LGPL)
------------------------------------------------------------
	glSDL 0.5 - SDL 2D API on top of OpenGL
------------------------------------------------------------
...
DBG(static void _print_glerror(int point) {
    const char *err = "<unknown>"; switch (glGetError()) {
case GL_NO_ERROR:
return; case GL_INVALID_ENUM:
err = "GL_INVALID_ENUM"; break; case GL_INVALID_VALUE:
err = "GL_INVALID_VALUE"; break; case GL_INVALID_OPERATION:
err = "GL_INVALID_OPERATION"; break; case GL_STACK_OVERFLOW:
err = "GL_STACK_OVERFLOW"; break; case GL_STACK_UNDERFLOW:
err = "GL_STACK_UNDERFLOW"; break; case GL_OUT_OF_MEMORY:
err = "GL_OUT_OF_MEMORY"; break; default:
    err = "<unknown>"; break;}
...
/* Allocate a "blank" texinfo for a suface. */
	glSDL_TexInfo *glSDL_AllocTexInfo(SDL_Surface * surface) {
		...
		if (!texinfotab) {
			texinfotab =
			    calloc(MAX_TEXINFOS, sizeof (glSDL_TexInfo *));
			if (!texinfotab)
				return NULL;
		}
 ...
 */
	static int _CalcChop(glSDL_TexInfo * txi) {
		...
		if (vh > maxtexsize) {
			fprintf(stderr,
				"glSDL: \"Huge\" surfaces not yet supported!\n");
			return 0;
		}
		...
		DBG3(fprintf
		     (stderr, "==> texsize=%d (rows=%d) \t", texsize, rows));
		...
		txi->textures =
		    (rows + txi->tilespertex - 1) / txi->tilespertex;
		...
		if (!txi->texture) {
			fprintf(stderr,
				"glSDL: INTERNAL ERROR: Failed to allocate"
				" texture name table!\n");
			return -2;
		}
...
----------------------------------------------------------*/
	...
	SDL_Surface *glSDL_SetVideoMode(int width, int height, int bpp,
					Uint32 flags) {
		SDL_Surface *screen;
		GLint gl_doublebuf;
		if (!(flags & SDL_GLSDL)) {
 ...
 */
		...
		DBG(printf("real = %d x %d\n", width * scale, height * scale);
		    ...
		    screen =
		    SDL_SetVideoMode(width * scale, height * scale, bpp, flags);
		if (!screen)
			return NULL;
		...
		if (glSDL_AddTexInfo(screen) < 0) {
			DBG(fprintf
			    (stderr,
			     "glSDL: Failed to add info to screen surface!\n"));
			SDL_QuitSubSystem(SDL_INIT_VIDEO);
			return NULL;
		}
	...
	void glSDL_UpdateRects(SDL_Surface * screen, int numrects,
			       SDL_Rect * rects) {
...
*/
	...
	void glSDL_UpdateRect(SDL_Surface * screen, Sint32 x, Sint32 y,
			      Uint32 w, Uint32 h) {
		SDL_Rect r;
		r.x = x;
		r.y = y;
...
*/
	...
	int glSDL_LockSurface(SDL_Surface * surface) {
		...
		if (IS_GLSDL_SURFACE(surface)) {
			if ((surface == fake_screen) ||
				...
				glReadPixels(0, 0, fake_screen->w,
					     fake_screen->h, GL_RGB,
					     GL_UNSIGNED_BYTE,
					     fake_screen->pixels);
				return 0;
			} else {
	...
	SDL_bool glSDL_SetClipRect(SDL_Surface * surface, SDL_Rect * rect) {
		...
		if ((screen == surface) && IS_GLSDL_SURFACE(surface)) {
			float xscale, yscale;
	...
	static __inline__ void gl_blendfunc(GLenum sfactor, GLenum dfactor) {
		if ((sfactor == glstate.sfactor)
		    && (dfactor == glstate.dfactor))
			return;
...
*/
		...
		if (srcrect)
			glReadPixels(srcrect->x, srcrect->y, srcrect->w,
				     srcrect->h, GL_RGB, GL_UNSIGNED_BYTE,
				     fake_screen->pixels);
		else
	...
	static void _BlitGL_vtile(glSDL_TexInfo * txi,
		...
		float xo = tile * tilew;
		int tex =
		    ((int) tile * txi->tilew + txi->tilew - 1) / txi->texsize;
	...
	static int _glSDL_BlitGL(SDL_Surface * src, SDL_Rect * srcrect,
		...
		case GLSDL_TM_SINGLE:
			_BlitGL_single(txi, x1, y1, x2, y2,
				       dx1, dy1, dx2, dy2, alpha, texscale);
			break;
...
*/
				_glSDL_BlitFromGL(srcrect, fake_screen,
						  dstrect);
				return _glSDL_BlitGL(fake_screen, srcrect, dst,
						     dstrect);
			} else {
			...
			if (dst == vs) {
				...
				glSDL_Invalidate(dst, dstrect);
				return SDL_BlitSurface(src, srcrect, dst,
						       dstrect);
			}
		}
	}
	...
	SDL_Surface *glSDL_DisplayFormat(SDL_Surface * surface) {
		...
		if (USING_GLSDL) {
			...
			if (use_rgba)
				tmp =
				    SDL_ConvertSurface(surface, &_RGBAfmt,
						       SDL_SWSURFACE);
			else
				tmp =
				    SDL_ConvertSurface(surface, &_RGBfmt,
						       SDL_SWSURFACE);
			if (!tmp)
				return NULL;
...
----------------------------------------------------------*/
	...
	void glSDL_SetLogicSize(SDL_Surface * surface, int w, int h) {
		...
		if ((SDL_GetVideoSurface() != surface)
		    && (fake_screen != surface))
			return;
...
/* Upload a single texture. */
	static int _UploadTexture(SDL_Surface * datasurf, glSDL_TexInfo * txi,
				  int tex) {
		...
		case GLSDL_TM_VERTICAL:
			{
				...
				while (tox + txi->tilew <= txi->texsize) {
					...
					if (thisth > txi->tileh)
						...
						break;
					glTexSubImage2D(GL_TEXTURE_2D, 0, tox,
							0, txi->tilew, thisth,
							datasurf->format->
							Amask ? GL_RGBA :
							GL_RGB,
	...
	int glSDL_UploadSurface(SDL_Surface * surface) {
		...
		if (txi->texsize > maxtexsize) {
			fprintf(stderr,
				"glSDL: INTERNAL ERROR: Too large texture!\n");
