/*
 * exemplu ptrace pentru urmarirea unui process - i386
 *
 * argumente: "pidwatcher comanda argumente" pentru a urmarirea comenzii
 *            "pidwatcher -p N" pentru a urmari procesul cu pid-ul N
 */
#include <errno.h>
#include <stdio.h>
#include <string.h>
...
/*
 * Lista de apeluri de sistem luat din <asm/unistd.h>
 */
...
/*
 * -1 este valid trebuie testat si valoarea errno
 */
...
pid_t pid;	/* pid-ul programului urmarit */

/* DETACH de la process cand acesta este intrerupt */
...
main(int argc, char **argv, char **envp){
	...
	if (argc >= 3 && !strcmp(argv[1], "-p")) {
		 ...
		 */
		my_ptrace_void(PTRACE_CONT, p, 0, 0);
	} else {
		void (*oldsig)(int);
		...
		my_ptrace_void(PTRACE_CONT, p, 0, (void *) SIGUSR1);
	}
